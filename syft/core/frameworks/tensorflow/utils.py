"""Tensorflow static utility functions."""
import json
import re
import types
import functools
...
def get_child_command(obj, child_types=[]):
    ...
    :return:
    """
    # Tensorflow tensor or variable, or sy._SyftTensor
    if (is_tensor(obj) or is_variable(obj) or is_syft_tensor(obj)) and not isinstance(obj, str):
        ...
        # TODO: What are the risks due to this assimilation? (see usage @ tf/utils.py l.74)
...
def prepare_child_command(command, replace_tensorvar_with_child=False):
    ...
    if len(next_child_types) == 0:
        ...
        if all([child_type in tf.tensor_types for child_type in next_child_types]):
...
def enforce_owner(obj, owner):
    ...
    if isinstance(obj, (list, tuple, set, bytearray)):
        ...
        if isinstance(obj, np.ndarray):
            ...
            # Tensorflow is circular this creates an infinite recursion. TODO: fix after Tensorflow 1.0
...
def wrap_command(obj):
    """
    To a Syft command, add a tf wrapper
    Returns the wrapper
    """
...
def assert_has_only_tf_tensorvars(obj):
    """
    A check function that an object has only tf Tensors or Variable
    ...
    if isinstance(obj, (int, float, str, slice)):
        ...
        rep = [assert_has_only_tf_tensorvars(o) for o in obj]
        return all(rep)
    elif isinstance(obj, dict):
        rep = [assert_has_only_tf_tensorvars(o) for o in obj.values()]
        return all(rep)
    elif callable(obj):
        return True
    elif obj is None:
...
def find_tf_object_in_family_tree(obj):
    """
    Returns the tf_objects on a local_chain (without pointer)
    """
    ch = obj.child
    while True:
...
def is_tensor_empty(obj):
    # TODO Will break with PyTensorflow >= 0.4
    return obj.dim() == 0
...
def is_tensor(obj):
    """
    Determines whether the arg is a subclass of a Tensorflow Tensor
    or is the name of a subclass of a Tensorflow Tensor
    """
    if isinstance(obj, str):
        if obj in map(lambda x: x.__name__, tf.tensor_types):
            return True
    else:
        if isinstance(obj, tuple(tf.tensor_types)):
            return True
    return False
...
def is_variable(obj):
    ...
    """
    if isinstance(obj, str):
        if obj in list(map(lambda x: x.__name__, tf.var_types)) + ['syft.Variable',
                                                                      'syft.Parameter']:
            return True
    else:
        if isinstance(obj, tuple(tf.var_types)):
            return True
    return False
