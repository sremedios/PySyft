...
from .tensor import _SyftTensor, _LocalTensor, _PointerTensor, _GeneralizedPointerTensor,_FixedPrecisionTensor, _TensorflowTensor
from .tensor import _TensorflowVariable


class TensorflowHook(object):
    r""" A Hook which Overrides Methods on Tensorflow Variables & Tensors -
 
     The purpose of this class is to:
 
         * extend tensorflow methods to allow for the moving of tensors
           and variables from one worker to another
         * override tf methods to execute commands on one worker
     ...
     using PySyft with Tensorflow because it is responsible for augmenting
     Tensorflow with PySyft's added functionality (such as remote execution).
     
     :Parameters:
 
         * **local_worker (**:class:`.workers.BaseWorker` **, optional)**
           you can optionally provide a local worker as a parameter which
           TensorflowHook will assume to be the worker owned by the local machine.
           If you leave it empty, TensorflowClient will automatically initialize
           a :class:`.workers.VirtualWorker` under the assumption you're
           looking to do local experimentation/development.
 
         * **is_client (bool, optional)** whether or not the TensorflowHook is
     ...
     >>> x
      ...
      2
      3
     [syft.core.frameworks.tensorflow.tensor.Tensor of size 6]
     """
    def __init__(self, local_worker=None, is_client=True, verbose=True, queue_size=0):
        self.local_worker = local_worker

        if not hasattr(tf, 'tensorflow_hooked'):
            tf.tensorflow_hooked = 0
        else:
            tf.tensorflow_hooked += 1
        ...
        if tf.tensorflow_hooked > 0:
            logging.warn("Tensorflow was already hooked... skipping hooking process")
            self.local_worker = sy.local_worker
        else:

            if self.local_worker is None:
                # Every TensorflowHook instance should have a local worker which is responsible for
                # interfacing with other workers. The worker interface is what allows the Tensorflow
                # specific code in TensorflowHook to be agnostic to the means by which workers communicate
    ...
    def _hook_native_tensors_and_variables(self, tensor_type):
        ...
        self._add_methods_from__TensorflowObject(tensor_type)
    ...
    def _add_registration_to___init__(hook_self, tensorvar_type, register_child_instead=False):
        ...
        # TODO: This is added because of the following contradiction: instanciate x = Tensor()
        # and ask x.__module__, you'll get `sy.core.frameworks.tf.tensor.Tensor`
        # but now ask for dir(sy.core.frameworks.tf.tensor) you'll find no Tensor attribute
        ...
        if tensorvar_type.__module__ != sy._SyftTensor.__module__:
            setattr(sy.core.frameworks.tensorflow.tensor, tensorvar_type.__name__, tensorvar_type)
    ...
    def _hook_properties(hook_self, tensor_type):
        ...
        # TODO: this should not be possible, but it should also be possible to define a Tensor
    ...
    def _which_methods_should_we_auto_overload(self, tensor_type=tf.Tensor):
    ...
    def _assign_methods_to_use_child(self, tensor_type):
        ...
        for attr in self.to_auto_overload[tensor_type]:
            ...
            def forward_method_to_child(self, *args, **kwargs):
                ...
                child_args = tf_utils.get_child_in_args(*args, **kwargs)
    ...
    def _add_methods_from__TensorflowObject(self, tensor_type):
        ...
        if issubclass(tensor_type, tf.Tensor):
            parent_syft_obj = _TensorflowTensor
        else:
            parent_syft_obj = _TensorflowVariable
        ...
        for attr in self.to_auto_overload[tensor_type]:
            ...
            def forward_method_to_child(self, *args, **kwargs):
                ...
                syft_node = type(self)(child=response.child, parent=None,
                                       tf_type=type(response).__name__)
    ...
    def _get_overloaded_method(hook_self, attr):
        """
        Wrapper overloading partial objects of methods in the tf
        ...
        def _execute_method_call(self, *args, **kwargs):
            ...
            try:
                ...
                if(not tf_utils.is_tensor(self)):
    ...
    def _hook_tf_module(self):
        """
        Overloads functions in the main tf module.

        The way this is accomplished is by first moving all existing module functions in the tf
        module to native_<function_name_here>. Thus, the real :func:`tf.cat` will become
        :func:`tf.native_cat` and :func:`tf.cat` will have our hooking code.
        """

        for module_name, module_funcs in tf.tf_modules.items():
            tf_module = eval(module_name)
            for attr in module_funcs:
                # Some functions we want to ignore (not override). Such functions have been hard
                # coded into the attribute self.tf_exclude
                if attr in tf.tf_exclude:
                    continue

                # if we haven't already overloaded this function
                if 'native_{}'.format(attr) in dir(tf_module):
                    continue
    ...
    def _hook_backward(hook_self):
        ...
        sy.Variable.native_native_backward = sy.train.GradientDescentOptimizer

        def new_backward(self, *args, **kwargs):
            worker = self.owner
            # Retrieve all the variable ids involved in the computation graph
            variable_ids = tf_utils.get_connected_variables(self)
            ...
            for variable_id in variable_ids:
                ...
                tf_utils.enforce_owner(var, worker)
                # Fix the .data and .grad attributes on the chain
                tf_utils.link_var_chain_to_data_and_grad_chains(var, var.data, var.grad)

        sy.Variable.native_backward = new_backward

    def _hook_module(self):
        """Overloading for tf.nn.Module"""        
        ...
        def module_send_(self, dest):
            """Overloads send to remote for tf.nn.Module"""
        ...
        hub.Module.send = module_send_

        def module_get_(self):
            """Overload get from remote for tf.nn.Module"""
            for p in self.parameters():
                p.get()

        hub.Module.get = module_get_
